...
function [x_1,x_2] = find_equilibrium_points(x1_dot, x2_dot)
  ...
  #disp(ans)
  for i =1:length(ans),
    x_1(i)=double(ans{i}.x1);
    x_2(i)=double(ans{i}.x2);
  endfor;
  x_1=x_1';
  #disp(x_1);
  x_2=x_2';
...
function jacobian_matrices = find_jacobian_matrices(x_1, x_2, x1_dot, x2_dot)
  ...
  solutions = [x_1, x_2];
  jacobian_matrices = {};
  j=jacobian([x1_dot,x2_dot],[x1;x2]);
  #disp(j)
  #disp(solutions)
  for i= 1:length(x_1)
    jac=subs(j,[x1 x2],[x_1(i) x_2(i)])
    jacobian_matrices{i}=[double(jac(1,1)),double(jac(1,2));double(jac(2,1)),double(jac(2,2))]
  endfor;
...
#jacobian_matrices=find_jacobian_matrices(x_1,x_2,x1_dot,x2_dot)
...
#check_eigen_values(x_1, x_2, jacobian_matrices)
