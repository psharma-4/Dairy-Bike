...
function dy = RW_pendulum_dynamics(y, m1, m2, l1, wr, g, u)
  ...
  disp(I2)
  dy(1,1) = y(2);
  dy(2,1) = -((m1*g*l1/2+m2*g*l1)/(I1+m1*l1*l1/4+m2*l1*l1))*sin(y(1))- (u/(I1+m1*l1*l1/4+m2*l1*l1));
  dy(3,1) = y(4);
  dy(4,1) = -dy(2,1)+u/I2;
endfunction
...
function [A, B] = RW_pendulum_AB_matrix(m1 , m2, l1, wr, g)
  I1= (m1*l1*l1)/3; # Moment of Inertia of pendulum bar
  I2= m2*wr*wr/2; # Moment of Inertia of reaction wheel
  A = [0 1 0 0;(m1*g*l1/2+m2*g*l1)/(I1+m1*l1*l1/4+m2*l1*l1) 0 0 0;0 0 0 1;-((m1*g*l1/2+m2*g*l1)/(I1+m1*l1*l1/4+m2*l1*l1)) 0 0 0];
  B = [0;-1/(I1+m1*l1*l1/4+m2*l1*l1);0;(1/(I1+m1*l1*l1/4+m2*l1*l1)+1/I2)];  
endfunction
...
function [t,y] = pole_place_RW_pendulum(m1, m2, l1, wr, g, y_setpoint, y0)
  [A,B] = RW_pendulum_AB_matrix(m1, m2, l1, wr, g);                            ## Initialize A and B matrix
  eigs = [-20;-12;-9;-19];                             ## Initialise desired eigenvalues
  K = place(A,B,eigs);
  tspan = 0:0.1:10;
...
function [t,y] = lqr_RW_pendulum(m1, m2, l1, wr, g, y_setpoint, y0)
  [A,B] = RW_pendulum_AB_matrix(m1,m2,l1,wr,g);               ## Initialize A and B matrix
  Q = [10 0 0 0;0 1 0 0;0 0 10 0;0 0 0 1];                   ## Initialise Q matrix
  R = [1];                   ## Initialise R 
  K = lqr(A,B,Q,R);
  disp(K)
